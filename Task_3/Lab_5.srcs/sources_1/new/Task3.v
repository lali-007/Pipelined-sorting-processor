...
module Task3(
    ...
    // This is to decide the correct function 
    wire [3:0] ALU_Control_Funct_ID = {Instruction_ID[30], funct3};

    // --- Control Unit outputs ---
    wire Branch_ID, MemRead_ID, MemtoReg_ID, MemWrite_ID, ALUSrc_ID, RegWrite_ID;
    wire [1:0] ALUOp_ID;

    // --- Checking whether there is a stall or not and if there is a stall it makes all the control signals 0.
    ...
    // Deciding whether to select branch or normal PC addition
    ...
    registerFile RF (
        ...
        .RS1(rs1_ID),
        .RS2(rs2_ID),
        .RD(rd_WB),          // WB stage wala
        .WriteData(WriteData_WB), // WB stage wala
        .RegWrite(RegWrite_WB),   // WB stage wala
    ...
    wire Is_BLT = (Funct_EX[2:0] == 3'b100) || (Funct_EX[2:0] == 3'b101); // Check Funct3 for BLT/BGE
