...
module Lab_11(
    ...
    wire [3:0]  operation_EX;
    wire [63:0] Adder_2_EX;   // Branch Target
    wire [63:0] Alu_seco_EX;  // Mux output (Imm vs Reg)
    
    // EX Forwarding Wires
    wire [1:0]  Forward_A, Forward_B;
    reg  [63:0] ALU_Input_A_Final;       // Output of Forward A Mux
    reg  [63:0] ALU_Input_B_Forwarded;   // Output of Forward B Mux
    ...
    registerFile RF(
        ...
        .RS1(rs1_ID), 
        .RS2(rs2_ID), 
        .RD(rd_WB),              // Loopback from WB
        .WriteData(WriteData_WB),// Loopback from WB
        .RegWrite(RegWrite_WB),  // Loopback from WB
        .ReadData1(ReadData1_ID), 
        .ReadData2(ReadData2_ID)
    );
...
// 2. FORWARDING UNIT
    // Using the specific port names from your Fowarding_Unit.v file
    Fowarding_Unit Fwd_Unit(
        .ID_EX_RS1(rs1_EX),             // Current Stage Source 1
        .ID_EX_RS2(rs2_EX),             // Current Stage Source 2
        .ID_EX_RD(rd_EX),               // Current Stage Dest (required by port list)
        .EX_MEM_RD(rd_MEM),             // Memory Stage Dest
        .MEM_WB_RD(rd_WB),              // Writeback Stage Dest
        .EX_MEM_REGWRITE(RegWrite_MEM), // Memory Stage Write Enable
        .MEM_WB_REGWRITE(RegWrite_WB),  // Writeback Stage Write Enable
        .FA(Forward_A),                 // Output to Mux A
        .FB(Forward_B)                  // Output to Mux B
    );

    // 3. FORWARDING MUX LOGIC (3-to-1 Muxes)
    // Forward A (ALU Input 1)
    always @(*) begin
        case (Forward_A)
            2'b00: ALU_Input_A_Final = ReadData1_EX;         // No Forwarding
            2'b10: ALU_Input_A_Final = ALU_Result_MEM;       // Forward from MEM
            2'b01: ALU_Input_A_Final = WriteData_WB;         // Forward from WB
            default: ALU_Input_A_Final = ReadData1_EX;
        endcase
    end

    // Forward B (Input to ALUSrc Mux)
    always @(*) begin
        case (Forward_B)
            2'b00: ALU_Input_B_Forwarded = ReadData2_EX;     // No Forwarding
            2'b10: ALU_Input_B_Forwarded = ALU_Result_MEM;   // Forward from MEM
            2'b01: ALU_Input_B_Forwarded = WriteData_WB;     // Forward from WB
            default: ALU_Input_B_Forwarded = ReadData2_EX;
        endcase
    end

    // 4. ALUSrc Mux (Immediate vs Forwarded Register Data)
    Multiplexer Mux_Reg(
        .a(ALU_Input_B_Forwarded), // Use the forwarded result here!
        .b(imm_data_EX), 
        .selector(ALUSrc_EX), 
        .data_out(Alu_seco_EX)
    );
    ...
    // 6. ALU
    ALU_64 ALU(
        .a(ALU_Input_A_Final),  // Use the forwarded result here!
        .b(Alu_seco_EX), 
        .ALUOp(operation_EX), 
        .Result(ALU_Result_EX), 
        .Zero(zero_EX), 
    ...
    // --- EX/MEM PIPELINE REGISTER ---
    // Note: We pass ALU_Input_B_Forwarded as ReadData2_in, 
    // because if we store to memory, we want the MOST RECENT data for that register.
    EX_MEM_reg ex_mem(
        .clk(clk),
        .reset(reset),
        // Control
        ...
        .ReadData2_in(ALU_Input_B_Forwarded), // Correct data for Store instructions
        .rd_in(rd_EX), .Funct_in(Funct_EX),
        // Outputs
        .Branch_out(Branch_MEM), .MemRead_out(MemRead_MEM), .MemtoReg_out(MemtoReg_MEM),
        .MemWrite_out(MemWrite_MEM), .RegWrite_out(RegWrite_MEM),
        .Adder_2_out(Adder_2_MEM), .zero_out(zero_MEM), .BLT_out(BLT_MEM),
    ...
    // 2. Data Memory
    wire [63:0] val1, val2, val3, val4, val5, val6, val7; 
